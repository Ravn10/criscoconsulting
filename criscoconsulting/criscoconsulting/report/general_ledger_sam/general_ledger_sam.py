...
def get_gl_entries(filters):
	...
	group_by_condition = "group by voucher_type, voucher_no, account, cost_center, branch" \
		if filters.get("group_by_voucher") else "group by acc.name"
	...
	gl_entries = frappe.db.sql("""
		select
			...
			gl.remarks, gl.against, gl.is_opening {select_fields},gl.narration,
			acc.branch
		from `tabGL Entry` as gl left join `tabAccount` as acc
