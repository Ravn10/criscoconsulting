...
from criscoconsulting.criscoconsulting.report.accounts_receivable_sam.accounts_receivable_sam import ReceivablePayableReport
...
class AccountsReceivableSummary(ReceivablePayableReport):
	...
	def get_columns(self, party_naming_by, args):
		...
		columns += [
			...
			str(self.filters.range4) + "-" + str(self.filters.range5) + ":Currency/currency:100",
			str(self.filters.range5) + "-" + str(self.filters.range6) + ":Currency/currency:100",
			str(self.filters.range6) + _("-Above") + ":Currency/currency:100"]
	...
	def get_data(self, party_naming_by, args):
		...
		for party, party_dict in partywise_total.items():
			...
			row += [
				...
				party_dict.range1, party_dict.range2, party_dict.range3, party_dict.range4,party_dict.range5,party_dict.range6,
				party_dict.range7
			]
	...
	def get_voucherwise_data(self, party_naming_by, args):
		...
		"outstanding_amt", "age", "range1", "range2", "range3", "range4", "range5", "range6", "range7", "currency"]
