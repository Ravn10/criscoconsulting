...
class PaymentReconciliationMulticurrency(Document):
	...
	def reconcile(self, args):
		...
		for e in self.get('payments'):
			if e.invoice_number and e.allocated_amount:
				doc_to_paid=frappe.get_doc(e.invoice_type,e.invoice_number)
				lst.append(frappe._dict({
					...
					'exchange_rate': flt(doc_to_paid.conversion_rate),
				}))
				
		if lst:
			from criscoconsulting.custom_payment_reconciliation_helper import reconcile_against_document
