...
frappe.ui.form.on("Bank Entry", {
	validate: function(frm){
		...
		var non_ar_receipt=frm.doc.non_ar_receipt;
		if(non_ar_receipt==1){
			switch(frm.doc.branch) {
					case 'Alkhobar':
						frm.set_value("naming_series","ARCPT-3-");
						break;
					case 'Riyadh':
						frm.set_value("naming_series","RRCPT-1-");
						break;
					case 'Jeddah':
						frm.set_value("naming_series","JRCPT-2-");
						break;
					case 'Head Office Jeddah':
						frm.set_value("naming_series","HRCPT-2-");
						break;
					case 'Car Division Jeddah':
						frm.set_value("naming_series","CDRCPT-2-");
						break;
					// default:
			...
			else
			{
			 switch(frm.doc.branch) {
					case 'Alkhobar':
						...
						// frappe.msgprint("Bank Payment Not Permitted" );
						// frm.disable_save();
						break;
					case 'Riyadh':
					...
					case 'Car Division Jeddah':
						 // frappe.msgprint("Bank Payment Not Permitted" );
						// frm.disable_save();
						frm.set_value("naming_series","CDBPV-2-");
						break;
					// default:
					// 	frm.set_value("naming_series","");
				}
	...
	refresh: function(frm) {
		...
		if (frm.doc.__islocal) {
			...
			//  reset journal entry , Total Amount field [duplicate case]-by Pranali
            cur_frm.set_value("journal_entry",undefined)
            cur_frm.set_value("total_amount",undefined)
            cur_frm.set_value("current_series",undefined)
            cur_frm.refresh_field("journal_entry")
            cur_frm.refresh_field("total_amount")
            cur_frm.refresh_field("current_series")
		}

		// hide /unhide fields based on currency
		erpnext.journal_entry.toggle_fields_based_on_currency(frm);

	console.log("status>",cur_frm.doc.docstatus,"jv>",cur_frm.doc.journal_entry,"clear>",cur_frm.doc.clearance_date)
		if(cur_frm.doc.docstatus==1 && cur_frm.doc.journal_entry && !cur_frm.doc.clearance_date){
		//  update Clearence date -by Pranali
		console.log("calling_function...")
		frappe.call({
				"method":"criscoconsulting.common_code.update_clearance_date",
				"args":{
						"doc":cur_frm.doc
				},
				"async":false,
				callback: function(r){
					console.log("updated clearance Date.>",cur_frm.doc.clearance_date)
				}
			});
		}
	},
...
erpnext.accounts.JournalEntry = frappe.ui.form.Controller.extend({
	...
	setup_balance_formatter: function() {
		...
		$.each(["balance", "party_balance"], function(i, field) {
			var df = frappe.meta.get_docfield("Bank Entry Account", field, me.frm.doc.name);
...
cur_frm.cscript.voucher_type = function(doc, cdt, cdn) {
	...
	var update_jv_details = function(doc, r) {
		$.each(r, function(i, d) {
			var row = frappe.model.add_child(doc, "Bank Entry Account", "accounts");
			row.account = d.account;
...
frappe.ui.form.on("Bank Entry Account", {
...
frappe.ui.form.on("Bank Entry Account", "accounts_remove", function(frm) {
	cur_frm.cscript.update_totals(frm.doc);
});
...
$.extend(erpnext.journal_entry, {
	toggle_fields_based_on_currency: function(frm) {
		...
		$.each(field_label_map, function (fieldname, label) {
			var df = frappe.meta.get_docfield("Bank Entry Account", fieldname, frm.doc.name);
...
//To filter only bank account in bank field
...
frappe.ui.form.on("Bank Entry","bank", function(){
			for (var i =0; i < cur_frm.doc.accounts.length; i++){
					cur_frm.doc.accounts[1].account = cur_frm.doc.bank;
					}
						cur_frm.refresh_field('accounts')
});
