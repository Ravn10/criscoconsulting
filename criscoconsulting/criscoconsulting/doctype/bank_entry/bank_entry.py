...
from criscoconsulting.common_code import cancel_journal_entry,create_journal_entry,delete_jv,separate_name
	...
	def validate_against_jv(self):
		for d in self.get('accounts'):
			if d.reference_type=="Journal Entry":
				...
				against_entries = frappe.db.sql("""select * from `tabBank Entry Account`
...
def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql("""select jv.name, jv.posting_date, jv.user_remark
		from `tabJournal Entry` jv, `tabBank Entry Account` jv_detail
...
def get_outstanding(args):
	...
	if args.get("doctype") == "Journal Entry":
		...
		against_jv_amount = frappe.db.sql("""
			...
			from `tabBank Entry Account` where parent=%(docname)s and account=%(account)s {0}
...
# 	frappe.errprint("py function")
# 	doc=json.loads(doc)
# 	frappe.errprint(str(doc))
# 	jv_entry=frappe.get_doc("Journal Entry",doc["name"])
# 	if jv_entry.clearance_date:
# 		frappe.db.sql("""update `tab{0}` set clearance_date = '{1}', modified = '{2}' where name='{3}'""".format("Bank Entry", jv_entry.clearance_date, nowdate(), doc["name"]))
# 		frappe.db.commit()
# 		frappe.msgprint("bank Entry Updated.")
# 	return "Done"


	# frappe.msgprint("my function")
	# if doc.reference_of_bank_entry:
	# 	frappe.db.set_value("Bank Entry", doc.reference_of_bank_entry, "clearance_date", doc.clearance_date)
	# 	frappe.msgprint("Bank Entry Updated")


	# bank_doc=frappe.get_doc(,)
	# bank_doc.clearance_date=doc.clearance_date
	# bank_doc.save()
	# bank_doc.submit()