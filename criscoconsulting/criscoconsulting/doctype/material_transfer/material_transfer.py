...
# 		frappe.msgprint("hi")
		if self.cost_center:
			centre1 = self.cost_center
			if self.material_transfer_type=='Send':
				self.material_transfer_series = frappe.db.get_value("Cost Center", centre1 , "material_transfer_series")
			if self.material_transfer_type=='Receive':
				self.material_transfer_series = frappe.db.get_value("Cost Center", centre1 , "material_transfer_receive_series")
...
def make_stock_entry_custom(source_name, target_doc=None):
	...
	# frappe.errprint("doclist>>"+str(doclist.material_request))
	return doclist
