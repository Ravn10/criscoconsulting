...
//To filter only bank account in cash field

cur_frm.fields_dict['cash'].get_query = function(doc) {
	return {
		filters: {
			"account_type": 'cash',
			"is_group":'0'
...
cur_frm.set_df_property("cheque_date", "reqd", cur_frm.doc.voucher_type=="Bank Entry");


frappe.ui.form.on("Cash Entry", "cash", function () {
    for (var i = 0; i < cur_frm.doc.accounts.length; i++) {
        cur_frm.doc.accounts[0].account = cur_frm.doc.cash;
    }
        ...
        if (frm.doc.non_ar_receipt == 1) {
            ...
            switch (frm.doc.branch) {
                ...
                case 'Head Office Jeddah':
                    ...
                    break;
                case 'Car Division Jeddah':
                    frm.set_value("naming_series", "CDCPV-2-");
                    break;
            }
        }
...
erpnext.accounts.JournalEntry = frappe.ui.form.Controller.extend({
	...
	setup_balance_formatter: function() {
		...
		$.each(["balance", "party_balance"], function(i, field) {
			var df = frappe.meta.get_docfield("Cash Entry Account", field, me.frm.doc.name);
...
cur_frm.cscript.voucher_type = function(doc, cdt, cdn) {
	...
	var update_jv_details = function(doc, r) {
		$.each(r, function(i, d) {
			var row = frappe.model.add_child(doc, "Cash Entry Account", "accounts");
			row.account = d.account;
...
frappe.ui.form.on("Cash Entry Account", "accounts_remove", function(frm) {
	cur_frm.cscript.update_totals(frm.doc);
});
...
$.extend(erpnext.journal_entry, {
	toggle_fields_based_on_currency: function(frm) {
		...
		$.each(field_label_map, function (fieldname, label) {
			var df = frappe.meta.get_docfield("Cash Entry Account", fieldname, frm.doc.name);
